  ...
  >
    ...
    <div class="msg-box">
      ...
      <div class="score">分数: {{ score }}</div>
			<div class="tips">Tips: 游戏进度将在最后一次操作后5s保存</div>
    </div>
    <!-- E 游戏信息 -->
...
export default {
  ...
  methods: {
    ...
    createBlock() {
      ...
      while (this.getBlock({ x: block.x, y: block.y })) {
        // 去重
    ...
    move(dir) {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        let group = this.blocks
          ...
          .sort((a, b) =>
            this.groupSort(param.end, a[param.coordinate], b[param.coordinate])
          );
          ...
          switch (n) {
            ...
            default:
              ...
              if (block.num === prevBlock.num && !merge) {
                ...
                if (param.end === 0) {
                  if (
                    block[param.coordinate] !==
                    group[n - 1][param.coordinate] + 1
                  ) {
